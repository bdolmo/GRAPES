...
sub CBS {
   ...
   foreach my $file (@ratios) {
     ...
     my $segmentRscript = "$inputDir/$sample.CBS.R";
  	 ...
  	 open (OUT, ">", $tmpSegment)|| die " Unable to open $tmpSegment\n";
  	 ...
  	 while (my $line =<IN>) {
  		...
  		print OUT "$tmp[0]\t$tmp[1]\t$tmp[1]\t$tmp[3]\t$tmp[6]\n";
  		print OUT "$tmp[0]\t$tmp[2]\t$tmp[2]\t$tmp[3]\t$tmp[6]\n";
  	 }
  	 close IN;
  	 close OUT;
  	 ...
  	 if (!-e "$inputDir/SEGMENT_DATA/segmented.$sample.bed") {
  		 ...
  		 if ($type eq 'on-target' && $analysis eq 'gene-panel') {
  			...
  			if (-e "$inputDir/SEGMENT_DATA/PSCBS.$sample.bed.tsv") {
  				...
  				$cmd.= " | $::awk '{ print \$1\"\t\"\$2\"\t\"\$3\"\t\"\$4\"\t\"\$7\"\t\"\$13}' > $inputDir/SEGMENT_DATA/toplot.segmented.$sample.bed"; #t
  				system $cmd;
  			}
  		 }
  		 else {
  			if (-e "$inputDir/SEGMENT_DATA/segmented.$sample.tmp.bed") {
  				...
  				$cmd.= " | $::awk '{ print \$1\"\t\"\$2\"\t\"\$3\"\t\"\$4\"\t\"\$7\"\t\"\$13}' > $inputDir/SEGMENT_DATA/toplot.segmented.$sample.bed";
  			}
  		 }
  		 system($cmd);
  		 $j++;
